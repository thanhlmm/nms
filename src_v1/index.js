...
document.querySelector("form").onsubmit = async (event) => {
  ...
  let attachedNear = document.querySelector(
    'input[name="attachedNear"]:checked'
  ).value;
  ...
  if (!(await isAccountExist(toAccount.value))) {
    alert(
      `The account '${toAccount.value}' is not existed. Please enter the other account!`
    );
    return;
  }
...
document.querySelector("input#toAccount").oninput = (event) => {
  checkSubmitButton();
};
document.querySelector("input#title").oninput = (event) => {
  checkSubmitButton();
};
document.querySelector("textarea#content").oninput = (event) => {
  checkSubmitButton();
};
...
async function showNotificationByTransHash(transHash) {
  ...
  if (result && result.status && result.status.SuccessValue) {
    let returnValue = new Buffer.from(
...
async function showNotificationByResult(isSuccess) {
  if (isSuccess) {
    ...
    document.querySelector("[data-behavior=notification-error]").style.display =
      "block";

    // Remove notification again after css animation completes
    setTimeout(() => {
      document.querySelector(
        "[data-behavior=notification-error]"
      ).style.display = "none";
    }, 11000);
  }
}
...
function signedInFlow() {
  ...
  let accountLink = document.querySelector(
    "[data-behavior=notification] a:nth-of-type(1)"
  );
  accountLink.href = accountLink.href + window.accountId;
  accountLink.innerText = "@" + window.accountId;
  let contractLink = document.querySelector(
  ...
  accountLink = document.querySelector(
    "[data-behavior=notification-error] a:nth-of-type(1)"
  );
  accountLink.href = accountLink.href + window.accountId;
  accountLink.innerText = "@" + window.accountId;
  contractLink = document.querySelector(
    "[data-behavior=notification-error] a:nth-of-type(2)"
  );
  contractLink.href = contractLink.href + window.contract.contractId;
  contractLink.innerText = "@" + window.contract.contractId;
...
async function updateDataMessage(msg, isLoadPreItem = false) {
  let msgInfo = await message.depackMessage({
    title: msg.title,
    data: msg.data,
  });
...
async function updateInboxUI() {
  ...
  let inboxMessages = [];
  let indexInfo = getIndexInfo(
    inboxMsgNum,
    appInfo.inbox.currentPage,
    appInfo.itemPerPage
  );
...
function messageItemToHtml(type, msg) {
  ...
  itemHtml += `<br /><b>Time:</b> ${new Date(
    msg.timestamp / 10 ** 6
  ).toLocaleString()}`;
  itemHtml += `<br /><b>Title:</b> ${msg.msgData.title}`;
  itemHtml += `<br /><b>Content:</b>`;
  itemHtml += `<div style="padding:5px">${standardHtmlData(
    msg.msgData.content
  )}</div>`;
  return itemHtml;
}
...
function showSentMessages(sentMsgNum, sentMessages) {
  ...
  if (fromPosition <= toPosition) {
    document.querySelector(
      "#bodyTitleInfo"
    ).innerText = `(${fromPosition}-${toPosition}/${sentMsgNum})`;
  } else {
    document.querySelector("#bodyTitleInfo").innerText = "";
  }
  ...
  for (let idx = 0; idx < sentMessages.length; idx++) {
    ...
    if (msg.prevMsgItem) {
      ...
      itemHtml +=
        "<div class='originMessage'>" +
        messageItemToHtml("all", msg.prevMsgItem) +
        "</div>";
    }
    if (msg.id > 0) {
      itemHtml += `<div class="seperatorInMessage"></div>`;
  ...
  if (sentMessages.length > 0) {
    ...
    document.querySelector("#sentMessages").innerHTML =
      "You haven't sent any message.<br />Do you want to send a message to someone? Please click <a href='#' onclick='onClickMenuItem(\"compose\");'>here</a> to start!!!";
  }

  // Show page
  document.querySelector("#pagingInfo").innerHTML = getPageLinkStr(
    "sent",
    sentMsgNum
  );
}
...
function showInboxMessages(inboxMsgNum, inboxMessages) {
  ...
  if (toPosition >= fromPosition) {
    document.querySelector(
      "#bodyTitleInfo"
    ).innerText = `(${fromPosition}-${toPosition}/${inboxMsgNum})`;
  } else {
    document.querySelector("#bodyTitleInfo").innerText = "";
  }
  ...
  if (inboxMessages.length > 0) {
    ...
    document.querySelector("#inboxMessages").innerHTML =
      "You have no message.<br />Do you want to send a message to someone? Please click <a href='#' onclick='onClickMenuItem(\"compose\");'>here</a> to start!!!";
  }
...
async function updateComposeUI(action, msgType, msgIndex) {
  ...
  let msgItem =
    msgType && msgIndex != null && msgIndex >= 0
      ? appInfo[msgType].messages[msgIndex]
      : null;
  ...
  if (action == "reply") {
    ...
    document.querySelector("input#toAccount").value =
      msgType == "inbox" ? msgItem.from : msgItem.to;
    document.querySelector("input#title").value =
      "Re: " + msgItem.msgData.title;
    document.querySelector("#originMessage").style.display = "block";
    document.querySelector("#originMessageContent").innerHTML =
      messageItemToHtml(msgType, msgItem);
  } else if (action == "forward") {
    document.querySelector("#bodyTitle").innerText = "Forward the message";
    document.querySelector("input#toAccount").value = "";
    document.querySelector("input#title").value =
      "Fw: " + msgItem.msgData.title;
    document.querySelector("#originMessage").style.display = "block";
    document.querySelector("#originMessageContent").innerHTML =
      messageItemToHtml(msgType, msgItem);
  } else {
...
async function updateStaticInfoUI() {
  ...
  document.querySelector(
    "#staticsInfo"
  ).innerHTML = `<b>Statics</b>: ${staticsInfo.sentAccountNum} used accounts, ${staticsInfo.accountNum} total accounts and ${staticsInfo.messageNum} messages`;
}
